...
def link_distance(depth: int) -> float:
    ...
    if depth == 2:
        return base
    return base + (depth - 2) * 25.0


def max_ring_multiplier(depth: int) -> float:
    if depth <= 1:
        return 1.6
    if depth <= 3:
        return 2.1
...
def assign_positions(
    ...
    if children:
        ...
        max_multiplier = max_ring_multiplier(depth + 1)
...
def main() -> None:
    ...
    max_multiplier = max_ring_multiplier(1)
    extra_distance = base_distance * (max_multiplier - 1.0)
    ring_spacing = extra_distance / (rings - 1) if rings > 1 else 0.0
    for index, child in enumerate(children):
        leaves = counts[index]
        child_span = span * leaves / total_leaves if total_leaves else span / len(children)
